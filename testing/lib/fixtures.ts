export const ASSET_1 = '7fa4da69-739d-4e32-a107-1f0bfd4a544b'
export const ASSET_2 = 'spxlFPL8WNSAmwL07b0e4su2Wa1EEZzw'
export const ASSET_2_SLUG = 'mutante'
export const ASSET_3 = 'caiQ4wQRlXFiOtMrCO2D86gX1odpqeuj'
export const ASSET_4 = 'B0k0MsxaS8nvTMbndBvvAEsBnyL0I6vx'
export const ASSET_HIDDEN = 'CTgHDPNAjeRpdPYg89WeDYwqa5pXcEC2'
export const ASSET_HIDDEN_SLUG = 'killion'

export const BUNDLE_1 = 'ALJMgJbpNUxzzaGgx1PQzx0RpUNTZziS'
export const BUNDLE_2 = '0ZqVg1MSpHFSkyvj9yWAyDfC1sjwQAxQ'

export const BUNDLE_PUBLIC = '083e16ae-8f1d-4fd8-af48-e71bf757efd2'
export const BUNDLE_PRIVATE = 'a672b690-8027-47e5-889c-ef19f78d9a7d'

export const CREATOR_1 = '9dd2096c-54e6-4eca-a08b-cb6e6fa5a2a1'
export const CREATOR_1_SLUG = 'advl'
export const CREATOR_2 = '224d9090-2c19-4d3c-b43d-b101d4879a3b'
export const CREATOR_2_SLUG = 'gerrin-tramis'
export const CREATOR_3 = '7bffd388-aba7-4bd2-8472-b63fef449805'
export const CREATOR_3_SLUG = 'carlos-cara-alvarez'

export const USER1 = 'cd240b82-6e33-48e7-a561-eac54bfb1a6b'

export const PAYMENT_INTENT_COMPLETE_PID = 'pi_3JetbEBmiuJDOPG52a7nMT74'
export const COIN_PURCHASE_PENDING = 27

export const users = {
  TEST1: {
    username: 'test-user-01',
    password: 'test-password',
    id: 'cd240b82-6e33-48e7-a561-eac54bfb1a6b'
  },
  ADMIN1: {
    username: 'test-admin-1',
    password: 'test-password'
  },
  CREATOR1: {
    username: 'test-creator-1',
    password: 'test-password',
    id: '349b984e-8e2e-4a4b-993b-34df19189dbf'
  }
}
